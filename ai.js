...
class AIService {
    ...
    static async processNotes(title, content) {
    ...
    static async chatMentor(prompt, history) {
        const key = this.getOpenAIKey();
        if (!key) {
    ...
    static async critiqueDesign(mediaType, imageUrl) {
        const key = this.getOpenAIKey();
        if (!key) {
            return {
                score: 85,
    ...
    // 4. AI Quiz Generator
    static async generateQuiz(topic, count, difficulty) {
        const key = this.getOpenAIKey();
        if (!key) {
    ...
    static async generatePodcast(title, summary, voiceType, style) {
        const key = this.getOpenAIKey();
        if (!key) {
    ...
    static async generateVideo(title, summary, durationMinutes) {
        const key = this.getOpenAIKey();
        if (!key) {
            return [
                {
