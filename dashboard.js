...
router.get('/stats', auth_1.authenticateJWT, async (req, res) => {
    try {
        ...
        const notesCount = await db_1.default.note.count({ where: { userId, isArchived: false } });
        const quizAttemptsCount = await db_1.default.quizAttempt.count({ where: { userId } });
        const critiquesCount = await db_1.default.mediaCritique.count({ where: { userId } });
        const tasksRemaining = await db_1.default.studyTask.count({ where: { userId, isCompleted: false } });
        // Recent items
        const recentNotes = await db_1.default.note.findMany({
            where: { userId, isArchived: false },
            orderBy: { createdAt: 'desc' },
            take: 4
        });
        const recentCritiques = await db_1.default.mediaCritique.findMany({
            where: { userId },
            orderBy: { createdAt: 'desc' },
            take: 3
        });
        const upcomingTasks = await db_1.default.studyTask.findMany({
        ...
        const badges = [];
        if (notesCount >= 1)
            badges.push({ id: 'badge1', name: 'Note Architect', icon: 'Brain', description: 'Upload and process your first learning note.' });
        if (quizAttemptsCount >= 1)
            badges.push({ id: 'badge2', name: 'Arena Challenger', icon: 'Trophy', description: 'Attempt a practice quiz in the arena.' });
        if (critiquesCount >= 1)
            badges.push({ id: 'badge3', name: 'Pixel Critic', icon: 'Palette', description: 'Request design critique diagnostics.' });
        if (user.streakDays >= 3)
            badges.push({ id: 'badge4', name: 'Unstoppable', icon: 'Flame', description: 'Maintain a 3+ day study streak.' });
...
router.get('/admin-summary', auth_1.authenticateJWT, async (req, res) => {
    try {
        ...
        const totalNotes = await db_1.default.note.count();
        const totalQuizzes = await db_1.default.quiz.count();
        const totalCritiques = await db_1.default.mediaCritique.count();
